...
library(caret)
library(car)
Data<-read_excel("Event5.xlsx", sheet = "Surfer Stats - Event 5")
Data<-Data[,-c(1,2,3,6,7,8)]
Data$`Previous 4 Event Points`<-as.numeric(Data$`Previous 4 Event Points`)
Data[38,20]<-0
```
...
```{r}
DataTrain$`Previous 4 Event Points`<-as.numeric(DataTrain$`Previous 4 Event Points`)
library(glmnet)
